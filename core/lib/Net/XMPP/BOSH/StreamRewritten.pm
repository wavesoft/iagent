...
sub ConnectSocket 
{
    ...
    # If we specified a SID to clone from, fetch it now
...
sub Connect
{
    ...
    $self->{SIDS}->{newconnection}->{connectiontype} = "bosh"
    ...
    # HTTP
    #---------------------------------------------------------------------------
    if ($self->{SIDS}->{newconnection}->{connectiontype} eq "bosh")
    {
        #-----------------------------------------------------------------------
            ...
            !exists($self->{SIDS}->{newconnection}->{httpsproxyhostname}))
        {
            ...
            push @{$self->{SIDS}->{newconnection}->{socks}} =
            ...
            $connected = defined($self->{SIDS}->{newconnection}->{socks}->[0]);
            ...
            ($self->{SIDS}->{newconnection}->{ssl} == 0))
        {
            ...
            if ($connected)
            {
                ...
                $self->{SIDS}->{newconnection}->{socks}->[0]->syswrite($put,length($put),0);
                my $buff;
                $self->{SIDS}->{newconnection}->{socks}->[0]->sysread($buff,4*POSIX::BUFSIZ);
                my ($code) = ($buff =~ /^\S+\s+(\S+)\s+/);
                $self->debug(1,"Connect: Combo #1: buff($buff)");
                $connected = 0 if ($code !~ /2\d\d/);
            }
            ...
            ($self->{SIDS}->{newconnection}->{ssl} == 0))
        {
            ...
            if ($connected)
            {
                $self->{SIDS}->{newconnection}->{socks}->[0]->syswrite($connect,length($connect),0);
                my $buff;
                $self->{SIDS}->{newconnection}->{socks}->[0]->sysread($buff,4*POSIX::BUFSIZ);
                my ($code) = ($buff =~ /^\S+\s+(\S+)\s+/);
        ...
        return unless $self->{SIDS}->{newconnection}->{socks}->[0];

        $self->debug(1,"Connect: We are connected");

        if (($self->{SIDS}->{newconnection}->{ssl} == 1) &&
            (ref($self->{SIDS}->{newconnection}->{socks}->[0]) eq "IO::Socket::INET"))
        {
            $self->debug(1,"Connect: Convert normal socket to SSL");
            $self->debug(1,"Connect: sock($self->{SIDS}->{newconnection}->{socks}->[0])");
            $self->LoadSSL();
            $self->{SIDS}->{newconnection}->{socks}->[0] =
                IO::Socket::SSL::socketToSSL($self->{SIDS}->{newconnection}->{socks}->[0],
                                             {SSL_verify_mode=>0x00});
            $self->debug(1,"Connect: ssl_sock($self->{SIDS}->{newconnection}->{socks}->[0])");
            $self->debug(1,"Connect: SSL: We are secure") if ($self->{SIDS}->{newconnection}->{socks}->[0]);
        }
        return unless $self->{SIDS}->{newconnection}->{socks}->[0]};
    }

    else {
        $self->SetErrorCode("newconnection","Unknown connection type");
        return;
    }

    $self->debug(1,"Connect: Got a connection");

    $self->{SIDS}->{newconnection}->{socks}->[0]->autoflush(1);
...
sub OpenStream
{
    ...
    $sockid = 0 unless defined($sockid);
    ...
    if ($self->{SIDS}->{$currsid}->{connectiontype} eq "bosh")
    {
    ...
    $self->{SIDS}->{$currsid}->{select} =
        new IO::Select($self->{SIDS}->{$currsid}->{socks}->[sockid]);

    if (($self->{SIDS}->{$currsid}->{connectiontype} eq "tcpip") ||
            ($self->{SIDS}->{$currsid}->{connectiontype} eq "http"))
    {
        $self->{SELECT} = new IO::Select($self->{SIDS}->{$currsid}->{socks}->[sockid]);
        $self->{SOCKETS}->{$self->{SIDS}->{$currsid}->{socks}->[sockid]} = "newconnection";
    }

    if ($self->{SIDS}->{$currsid}->{connectiontype} eq "stdinout")
    {
        $self->{SELECT} = new IO::Select(*STDIN);
        $self->{SOCKETS}->{$self->{SIDS}->{$currsid}->{socks}->[sockid]} = $currsid;
