...
iAgent::Module::LDAPAuth - LDAP Authentication and Authorization for iAgent

=head1 DESCRIPTION

This module provides LDAP Authentication and Authorization for iAgent. 

It scans comm_action, comm_available, comm_unavailable dispatches, it validates the source
and it prohibits the propagation if they are invalid.
...
package iAgent::Module::LDAPAuth;
use iAgent::Log;
use iAgent::Kernel;
use strict;
use warnings;
...
############################################
    ...
    # Process permissions
    my $perms = {};
    foreach my $k (keys %{$config->{Permission}}) {
        $perms->{$k} = $config->{Permission}{$k}{LDAPGroup};
        $self->{DefaultPerm}->{$k} = ($config->{Permission}{$k}{Default} eq '0')?0:1;
        $self->{Permissions}->{$k} = $config->{Permission}{$k}{Description};
    }
            
    # Save config to local variable
    $self->{LDAPInfo} = {
        ...
        LDAPNameAttrib => $config->{LDAPNameAttrib},
        LDAPOverride => $config->{LDAPOverride},
...
############################################
sub ___start { # Intercept the start message
############################################
...
############################################
sub _DISABLED___ready { # System ready! All modules loaded
############################################
    my $self = $_[OBJECT];
    
    # Merge all the permission entries from all the manifests
    my %hash;
    for my $session (@iAgent::Kernel::SESSIONS) {
    	
    	# Fetch the session permissions if exists
    	my $heap = $session->get_heap();
    	if (defined $heap->{permissions}) {
    		# Merge hashes
    		my %n_hash = %{$heap->{permissions}};
            @hash{keys %n_hash} = values %n_hash;
    	}
    }
